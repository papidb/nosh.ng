...
export const SwipeButton = ({onToggle, loading, title}) => {
  ...
  const handleCompletePure = (isToggled) => {
    setToggled(isToggled);
  };
  ...
  useEffect(() => {
    if (
      ...
      console.log({loading, toggled, value: X.value});

      // console.log('taking x to 0');
      // console.log({loading, toggled, value: X.value});
      X.value = withSpring(0);
      runOnJS(handleCompletePure)(false);
    }
  }, [X.value, loading, toggled]);
